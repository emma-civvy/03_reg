...
ggplot(reg_dfw, 
       aes(x = sr_ksha, 
           y = yield_mgha))+
  geom_smooth()+
  geom_point() ## looks similar to quadratic model - potentially best model to apply.
...
options(contrasts = c("contr.sum", "contr.poly")) ## change matrix restriction. default is "set to zero". Build up of parameters is affected by this. "Sum to zero" is more applicable to research. 

# Model fitting
mod1_int <- lmer(yield_mgha ~ 1 + (1 | rep),
                 data = reg_dfw) ## 1 = intercept only model. average yield over reps 
...
mod1_int_aug <- augment(mod1_int) %>% 
  mutate(.stdresid = resid(mod1_int, ## get standardized residuals
                           type = "pearson",
                           scale = TRUE
                           )) %>% 
  left_join(reg_dfw) ## include the seeding rate treatments, which weren't accounted for in the models
...
## check if within group errors are independently normal (distributed around 0)
...
ggplot(mod1_int_aug,
  ...
  geom_hline(yintercept = 0,
             color = "red")+
  geom_smooth() ##residuals are increasing as fitted values are increasing. Not ideal - want to see No clear patters, just straight line like 0. 
## INdicates we may need to include some sort of slope.
```
...
ggplot(mod1_int_aug,
       aes(sample = .stdresid))+
  stat_qq()+
  stat_qq_line() ## assess if resids are normally distributed. Black line = IF they were normally distributed. Points = actual residuals. Some deviations at tails. 
...
ggplot(mod1_int_randeff,
       aes(sample =`(Intercept)`)) + ## use accent symbol under esc button
  stat_qq()+
  stat_qq_line()
```
...
ggplot(mod1_int_aug, 
  ...
  geom_line(aes(y = .fixed),
            color = "blue") ## this is what our model is doing. same yield value across all seeding rates. obviously not the case. 
```
...
```{r}
## bootstrap with replacement - will sample same number of rows but same observation can/will be sampled more than once. 
...
nd <- nd %>% 
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA)) ## same value of yield across everything (to be expected because int-only model)
...
```{r}
## does AIC weight/account for residuals?
```
...
# Summary
summary(mod2_lin) ## seeding rate is SLOPE value. 
...
ggplot(mod2_lin_aug, 
  ...
  geom_smooth() ## clear quadratic shape to residuals.
...
ggplot(mod2_lin_aug, 
  ...
  geom_line(aes(y = .fixed),
            color = "blue"
            ) ## want basically the same distance between actual points and modeled points
```
...
ggplot(mod3_quad_aug, 
  ...
  geom_hline(yintercept = 0, color = "red")+
  geom_point(size = 3, alpha = .7)+
  geom_smooth() ## one point pullinf things up, but overall looking better
```
...
ggplot(reg_dfw, 
       aes(x = sr_ksha, 
           y = yield_mgha))+
  geom_point(size = 3, alpha = .7)+
  geom_line(data = nd, ## layering predicted line over real values to create smoother curve. better understanding - change geom_line to geom_point. 
...
?SSlinp
# Model fitting
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha, ## helper function
                                      a, ## variable for int
                                      b, ## variable for slope
                                      xs), ## variable for break point
                  data = reg_dfw,
                  random = list(rep = pdDiag(a + b + xs ~ 1)), ## creates a diagonal matrix
                  fixed = list(a ~ 1,
                               b ~ 1,
                               xs ~ 1),
                  start = c(a = 0,
                            b = 0.3,
                            xs = 100
                            ) ## always visually determine slope/break point or is there a way to statistically identify? looking at quadratic coefficients/figures to provide value 
                  ## model is just using these as STARTING values. Need to be close but at some point it can take the reigns.
                  ) 
...
ggplot(mod4_linp_randeff, 
  ...
  stat_qq()+
  stat_qq_line()+
  facet_wrap(~term) ## would you remove the RE in this case?
...
IC_tab(mod1_int,
       mod2_lin,
       mod3_quad,
       mod4_linp) ## lowest = better (mod4/linear plateau)
## less than 2 points is not significantly different 
...
```{r optimum SR}
mod4_linp %>% 
  intervals(which = "fixed") ## xs = point optimum
## model does not tell you acheived seeding rate.
...
predict(mod4_linp, 
        newdata = data.frame(sr_ksha = 73.48), ## must align with data that model was trained on 
        level = 0 ## don't incorporate REs
        )
